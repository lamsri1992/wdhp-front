<form action="{{ route('form.addInfo',$patient->patient_id) }}">
    <div class="row">
            ...
            <select name="info_type" class="form-select">
                ...
                <optgroup label="สมัครใจ">
                    ...
                    <option value="{{ $res->type_group.",".$res->type_id }}"
                        {{ old('info_type') === $res->type_group.",".$res->type_id ? 'selected' : '' }}>
                        {{ $res->type_name }}
                    </option>
                    @endforeach
                </optgroup>
        ...
        <div class="form-group col-md-4">
            ...
            <div class="input-group mb-2">
                ...
                value="{{ old('info_date_in') }}" readonly>
        ...
        <div class="form-group col-md-4">
            ...
            <select class="basic-multiple" name="info_drug[]" multiple="multiple">
                ...
                <option value="{{ $res->m_type_id }}"
                    {{ (collect(old('info_drug'))->contains($res->m_type_id)) ? 'selected' : '' }}>
                    {{ $res->m_type_name }}
                </option>
                @endforeach
            </select>
        ...
        <div class="form-group col-md-4">
            ...
            <div class="input-group mb-2">
                ...
                value="{{ old('info_drug_use') }}">
                &nbsp;
                <div class="input-group-prepend">
                    <select name="info_drug_time" class="form-select" style="width: 100%;">
                        <option>- เลือก -</option>
                        <option value="d"
                        {{ old('info_drug_time') === 'd' ? 'selected' : '' }}>
                            วัน
                        </option>
                        <option value="m"
                        {{ old('info_drug_time') === 'm' ? 'selected' : '' }}>
                            เดือน
                        </option>
                        <option value="y"
                        {{ old('info_drug_time') === 'y' ? 'selected' : '' }}>
                            ปี
                        </option>
                    </select>
                </div>
        ...
        <div class="form-group col-md-4">
            ...
            <div class="input-group mb-2">
                ...
                value="{{ old('info_drug_last') }}">
                &nbsp;
                <div class="input-group-prepend">
                    <select name="info_last_time" class="form-select" style="width: 100%;">
                        <option>- เลือก -</option>
                        <option value="d"
                        {{ old('info_last_time') === 'd' ? 'selected' : '' }}>
                            วัน
                        </option>
                        <option value="m"
                        {{ old('info_last_time') === 'm' ? 'selected' : '' }}>
                            เดือน
                        </option>
                        <option value="y"
                        {{ old('info_last_time') === 'y' ? 'selected' : '' }}>
                            ปี
                        </option>
                    </select>
                </div>
        ...
        <div class="form-group col-md-12">
            ...
            <div class="input-group mb-2">
                <textarea name="info_pi" rows="5" class="form-control">{{ old('info_pi') }}</textarea>
            </div>
        </div>
    </div>
    <div class="form-group text-end">
        <button type="button" class="btn btn-success"
            onclick="Swal.fire({
                title: 'ลงทะเบียนผู้เข้ารับการบำบัดใหม่ ?',
